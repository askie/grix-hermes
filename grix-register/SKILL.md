...
```bash
node scripts/grix_auth.js send-email-code --email <EMAIL> --scene register
node scripts/grix_auth.js register --email <EMAIL> --password <PASSWORD> --email-code <CODE>
node scripts/grix_auth.js login --email <EMAIL> --password <PASSWORD>
node scripts/grix_auth.js create-api-agent --access-token <TOKEN> --agent-name <NAME>
```
